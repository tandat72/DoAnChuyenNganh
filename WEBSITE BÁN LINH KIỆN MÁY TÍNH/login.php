...
session_start();
	...
	if($_SERVER['REQUEST_METHOD'] == "POST")
	{
	    ...
	    if(empty($user_name)||empty($password))
	        {
	            echo '<p style="color:red"> Tên đăng nhập hoặc mật khẩu không được để trống!';
	        }
				$user = new classAll();
				$result = $user -> get_user($user_name);
				$row = mysqli_fetch_assoc($result);
				$_SESSION['user_name'] = $row['user_name'];
				$_SESSION['user_id'] = $row['user_id'];
				$_SESSION['user_email'] = $row['user_email'];
	            if($row['role_id'] == 1){
	  				echo '<p style="color:green"> Bạn đã đăng nhập thành công';
	  			echo "<script>window.location.href='admin/index.php'</script>";;
	            	...
	            	echo '<p style="color:green"> Bạn đã đăng nhập thành công';
	            	 ...
	            	 echo '<p style="color:red"> Tên đăng nhập hoặc mật khẩu không đúng!';
	            }
	}
...
form svg {
  ...
  position: relative;
	...
	<div id="box">
		...
		<form method="post">
			<div style="font-size: 20px;margin: 10px;color: white;">Đăng nhập</div>

			<input  class="text" type="text" placeholder="Tên tài khoản" name="user_name"><br><br>
			<input id="password" class="text" type="password" placeholder="Mật khẩu" name="password" onfocus="black()" onblur="white()">
      ...
      <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 59.049 59.049" xml:space="preserve" onclick="passwordHide()" id="hideEye">
        <g>
          ...
          <path d="M54.621,5.567c-0.391-0.391-1.023-0.391-1.414,0l-46.5,46.5c-0.391,0.391-0.391,1.023,0,1.414   c0.195,0.195,0.451,0.293,0.707,0.293s0.512-0.098,0.707-0.293l46.5-46.5C55.012,6.591,55.012,5.958,54.621,5.567z"/>
			...
			<br><br>
			<input id="button" class="text" type="submit" value="Đăng nhập"><br><br>

			<p>Chưa có tài khoản ấn vào <a href="signup.php">Đây</a> </p>
